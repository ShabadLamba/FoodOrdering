...
# def restaurantInfo():
#
from zomato_cuisine_restaurant import insertManyCuisines,findDistinctRestaurantIds
from zomato_locality import getAllIdsOfLocality
from zomato_restaurant import insertManyRestaurants,getAllRestaurantsLocalityId,findRestaurantIds
from zomato_location_restaurant import insertManyLocalityRestaurants,findAllLocalityIdsAndRestaurantIds
import subprocess
import os
import ast
...
def assignValueToAttributesOfRestaurants(restaurantData,id):
	...
	return (resId,name,url,address,id[0],latitude,longitude,rating,country_id,phone,timings,average_cost_for_two,is_pure_veg,\
		...
		try:
			...
			while start<results_found:
				...
				for restaurant in listOfRestraunt:	
					...
					if (id[0],str(restaurant["restaurant"]["id"]),) not in findAllLocalityIdsAndRestaurantIds():
						insertManyLocalityRestaurants([(id[0],restaurant["restaurant"]["id"],)])
					
					if (restaurant["restaurant"]["id"],) not in findRestaurantIds():
						insertManyRestaurants([assignValueToAttributesOfRestaurants(restaurant,id)])

					if (str(restaurant["restaurant"]["id"]),) not in findDistinctRestaurantIds():

						for cuisines in restaurant["restaurant"]["cuisines"].split(","):

							insertManyCuisines([(cuisines,restaurant["restaurant"]["id"],)])
